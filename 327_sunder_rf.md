<div class="container">
    <nav class="sidenav" id="nav-placeholder"></nav>
    <main class="content-wrapper">
        <div class="content-inner">
            <article>
<!-- START OF MAIN CONTENT -->

# 3.27 [Chieftain] Sunder Leveling into Pohx RF Swap
Updated: Feb 11, 2026

<div class="note-block">
<ul class="custom-list bullet-list">

- This guide adapts <a href="https://pohx.net/">Pohx's RF guide</a> by using Sunder for leveling instead of the traditional caster approach. Play Sunder until Level 52-55, then use the free campaign respec and gold at Blood Aqueducts to hard-swap into Righteous Fire and follow Pohx's instructions. This approach allows you to level significantly quicker while still transitioning smoothly into the powerful RF setup once you reach maps.
  
</ul>
</div>

## <span id="pob"></span>POB Links

<ul class="custom-list bullet-list">

- 3.27 Guide POB: <a href="https://pobb.in/FkhZiFT0WE4O">This Guide</a> <a href="important"> USE THIS</a>
- 3.26 Day 2 POB: <a href="https://pobb.in/GaPzv7hRoc9y">LVL 94 Starter</a>
- 3.26 Day 3 POB: <a href="https://pobb.in/h3DDjaATOe06">LVL 95 MB Swap</a>
- 3.26 Day 5 POB: <a href="https://pobb.in/bYrTsYELAH7e">LVL 98 Dawnbreaker + Stormshroud (ailment immune)</a>
- 3.26 Day 7 POB: <a href="https://pobb.in/muvlA8bo3mGJ">LVL 99 Crafted Shield</a>
- 3.26 Day 9 POB: <a href="https://pobb.in/GZ8yhzUkKXoi">LVL 99 Svalinn Swap</a>

</ul>

---

## <span id="regex"></span>REGEX
<div class="code-container">
    <div class="code-header">
        <span>Gear Regex - works for full campaign</span>
        <button class="copy-btn" onclick="copyCode(this)">Copy</button>
    </div>

```-\w-.-|r-r-r|g-[gr]-r|r-[gr]-g|g-r-g|r-g-r|Runn|s:.+ax```

</div>

<div class="code-container">
    <div class="code-header">
        <span>Gem Regex for only Acts 1-3</span>
        <button class="copy-btn" onclick="copyCode(this)">Copy</button>
    </div>

```"stbl|mida|omen|jade| pur|er att|ood r|determ|y of f"```

</div>

---

## <span id="a1"></span>ACT 1 PROGRESSION
### End of Act 1: 20-23 mins - Level 12

<img src="327_sunder/images/act1.png">

<ul class="custom-list bullet-list">

- Starting Gems: [Heavy Strike](gemred)+[Ruthless](gemred)
- Ensure you have [R-R](gemred) linked somewhere before leaving town
  
</ul>

<div class="note-block">
<ul class="custom-list bullet-list">

- Below are all breakpoints by monster, after each name go to town and get the items listed.
  
</ul>


</div>

### Tarkleigh
<ul class="checklist">

- [ ] Get [Ground Slam](gemred)(left gem) → Replace [Heavy Strike](gemred)
- [ ] Buy [Stone Axe](gemgrey) for cheap
- [ ] Buy any [R-R](gemred) linked
- [ ] Pick up any shield and put it on off-hand for [Shield Charge](gemred) at level 4 

</ul>

### Hailrake
<ul class="checklist">

- [ ] Get [Quicksilver](gemgrey)
- [ ] Get [Decoy Totem](gemred) (optional)
- [ ] Get [Shield Charge](gemred)
- [ ] Get [Chance to Bleed](gemred) or [Momentum](gemgreen) 
- I personally take [Momentum](gemgreen) and link with [Shield Charge](gemred)
- [ ] Buy [Jade Amulet / Iron Ring](gemgrey) 3x <img src="images/wisdom.png" class="icon-inline">
- [ ] Buy [Frostblink](gemblue) <img src="images/wisdom.png" class="icon-inline">
- [ ] Buy [Momentum](gemgreen) or [Chance to Bleed](gemred)<img src="images/wisdom.png" class="icon-inline">
- Buy the other gem you didn't get earlier
- [ ] Buy [Blood & Sand](gemred)<img src="images/wisdom.png" class="icon-inline">

</ul>

### Dweller
<ul class="checklist">

- [ ] Get [Faster Attacks](gemgreen)
- [ ] Buy 1-2x [Iron Rings](gemgrey) and 2x [mana potions](gemblue) to make Brutus fight easier

</ul>

### Brutus
<ul class="checklist">

- [ ] Get [Leap Slam](gemred)
- [ ] Get [Indimidating Cry](gemred)

</ul>

### Fairgraves (Enter Caverns)
<ul class="checklist">

- [ ] Get [Sunder](gemred)

</ul>

<div class="link-block" id="#a1links">
<ul class="checklist">

### Level 1-9: Use Ground Slam
- [ ] Link 1: [Ground Slam](gemred)+[Ruthless](gemred)+[Chance to Bleed](gemred) or [Momentum](gemgreen)
- [ ] Link 2: [Shield Charge](gemred)+[Momentum](gemgreen)
- [ ] Util: [Frostblink](gemblue), [Blood & Sand](gemred), [Intimidating Cry](gemred)

### Level 10: Replace Shield Charge with Leap Slam
- [ ] Link 2: [Leap Slam](gemred)+[Momentum](gemgreen)

### Level 12: Mervail
- [ ] Link 1: [Sunder](gemred)+[Ruthless](gemred)+[Chance to Bleed](gemred) or [Momentum](gemgreen)
- [ ] Link 2: [Leap Slam](gemred)+[Faster Attacks](gemgreen)+[Momentum](gemgreen)
- [ ] Util: [Frostblink](gemblue), [Blood & Sand](gemred), [Intimidating Cry](gemred)

Total Cost: 1 Transmute<img src="images/trans.png" class="icon-inline">, 3+ Wisdoms<img src="images/wisdom.png" class="icon-inline">
</ul>
</div>

---

## <span id="a2"></span>ACT 2 PROGRESSION
### End of Act 2: 38-40 mins, Level 20

<img src="327_sunder/images/act2.png">

 ### Fidelitas
 <ul class="checklist">
 
 - [ ] Get [Herald of Ash](gemred)
 - [ ] Buy [Blood Rage](gemgreen)<img src="images/alt.png" class="icon-inline">
 - [ ] Buy [Herald of Purity](gemred)<img src="images/alt.png" class="icon-inline"> (optional)
 - [ ] Buy [Jade Amulet](gemgreen)3x<img src="images/wisdom.png" class="icon-inline">
 </ul>

### Weaver
 <ul class="checklist">

- [ ] Get [Melee Physical Damage](gemred)
 </ul>

<div class="link-block" id="a2links">
<ul class="checklist">

- [ ] Link 1: [Sunder](gemred)+[Melee Physical Damage](gemred)+[Ruthless](gemred) or [Close Combat](gemgreen)
- [ ] Link 2: [Leap Slam](gemred)+[Faster Attacks](gemgreen)+[Momentum](gemgreen)
- [ ] Aura: [Herald of Ash](gemred)
- [ ] Util: [Frostblink](gemblue), [Blood & Sand](gemred), [Intimidating Cry](gemred), [Blood Rage](gemgreen)

Total Cost: 1-2 Alterations <img src="images/alt.png" class="icon-inline">, 3 Wisdoms <img src="images/wisdom.png" class="icon-inline">
</ul>
</div>

---

## <span id="a3"></span>ACT 3 PROGRESSION
### End of Act 3: 55-60 mins, Level 28-30

<img src="327_sunder/images/act3.png">

<div class="note-block">
You can gamble at <a href="npc">Maramoa</a> at level 24 for 4 Links. Focus on armor since it's used for a longer time than weapons which you should always be upgrading. <a href="important">DO NOT OVER SPEND</a> as you will need gold later to respec to RF in Act 9
<ul class="custom-list bullet-list">

- 2H Axe / body armor: 145 gold each
- Gloves / Helmet / Boots: 135 gold each <a href="important"> < BETTER OPTION</a>

</ul>

</div>

### Piety
<ul class="checklist">

- [ ] Get [Vulnerability](gemred)

</ul>

<div class="link-block" id="a3links">
<ul class="checklist">

- [ ] Link 1: [Sunder](gemred)+[Melee Physical Damage](gemred)+[Cruelty](gemred)+[Ruthless](gemred) or [Close Combat](gemgreen)
- [ ] Link 2: [Leap Slam](gemred)+[Faster Attacks](gemgreen)+[Momentum](gemgreen)
- [ ] Aura: [Herald of Ash](gemred)
- [ ] Util: [Frostblink](gemblue), [Blood & Sand](gemred), [Intimidating Cry](gemred), [Blood Rage](gemgreen), [Vulnerability](gemred)

Total Cost: None
</ul>
</div>

---

## <span id="a4"></span>ACT 4 PROGRESSION
### End of Act 4: 1h20, Level 35

<img src="327_sunder/images/act4.png">

<div class="note-block">
<ul class="custom-list bullet-list">

- Do Normal Lab once you reach <a href="npc">Crystal Veins</a> waypoint
- Take Chieftain - <a href="npc">Tasalio, Cleansing Water</a> for first ascendancy

</ul>
</div>

### Malachai
<ul class="checklist">

- [ ] Get [Fist of War](gemred) replace [Cruelty](gemred) or [Close Combat](gemgreen)

</ul>

<div class="link-block" id="a4links">
<ul class="checklist">

- [ ] Link 1: [Sunder](gemred)+[Melee Physical Damage](gemred)+[Cruelty/Ruthless](gemred)+[Fist of War](gemred)
- [ ] Link 2: [Leap Slam](gemred)+[Faster Attacks](gemgreen)+[Momentum](gemgreen)
- [ ] Aura: [Herald of Ash](gemred)
- [ ] Util: [Frostblink](gemblue), [Blood & Sand](gemred), [Intimidating Cry](gemred), [Blood Rage](gemgreen), [Vulnerability](gemred)

Total Cost: None
</ul>
</div>

---

## <span id="a5"></span>ACT 5 PROGRESSION
### End of Act 5: 1h35, Level 40

<img src="327_sunder/images/act5.png">

<div class="note-block">
<ul class="custom-list bullet-list">

- Act 5-9 will keep the same gem links until the respec at Blood Aqueducts in A9

</ul>
</div>

<div class="link-block" id="a5links">
<ul class="checklist">

- [ ] Link 1: [Sunder](gemred)+[Melee Physical Damage](gemred)+[Cruelty/Ruthless](gemred)+[Fist of War](gemred)
- [ ] Link 2: [Leap Slam](gemred)+[Faster Attacks](gemgreen)+[Momentum](gemgreen)
- [ ] Aura: [Herald of Ash](gemred)
- [ ] Util: [Frostblink](gemblue), [Blood & Sand](gemred), [Intimidating Cry](gemred), [Blood Rage](gemgreen), [Vulnerability](gemred)

Total Cost: None
</ul>
</div>

---

## <span id="a6"></span>ACT 6 PROGRESSION
### End of Act 6: 2h, Level 45

<img src="327_sunder/images/act6.png">

<div class="imp-block" id="a6links">
<ul class="checklist">
As soon as you finish <a href="npc">Twilight Strand quest</a> to unlock <a href="npc">Lilly Roth</a> make sure you buy these 6 gems and level them in the off-hand weapon. You will need these once you respec in Blood Aqueducts for the RF swap.

- [ ] [Fire Trap](gemgreen)<img src="images/trans.png" class="icon-inline">
- [ ] [Swift Assembly](gemgreen)<img src="images/wisdom.png" class="icon-inline">
- [ ] [Trap & Mine Damage](gemgreen)<img src="images/alt.png" class="icon-inline">
- [ ] [Elemental Focus](gemblue)<img src="images/alt.png" class="icon-inline">
- [ ] [Malevolence](gemblue)<img src="images/alt.png" class="icon-inline">
- [ ] [Burning Damage](gemred)<img src="images/chance.png" class="icon-inline">

Total Cost: 1 Transmute<img src="images/trans.png" class="icon-inline">, 1 Wisdom<img src="images/wisdom.png" class="icon-inline">, 3 Alterations<img src="images/alt.png" class="icon-inline">, 1 Chance<img src="images/chance.png" class="icon-inline">

</ul>
</div>

<div class="note-block">
<ul class="custom-list bullet-list">

- After killing <a href="npc">Shavronne</a> grab the <a href="important">helmet reward</a> from <a href="npc">Sin</a> in town that matches your builds armor type:
  - For Righteous Fire look for <a href="b">BBB</a><a href="r">R</a> links.
  - For Fire Trap look for <a href="g">GGG</a><a href="r">R</a> links.
- If the reward has exactly those links, take it. It's ready to equip with your corresponding gems once you swap builds.
  - Check wiki if needed: <a href="[filename](https://www.poewiki.net/wiki/Essence_of_Umbra)">Essence of Umbra Quest</a>

</ul>
</div>

## <span id="a7"></span>ACT 7 PROGRESSION
### End of Act 7: 2h15, Level 49

<img src="327_sunder/images/act7.png">

<div class="note-block">
<ul class="custom-list bullet-list">

- After killing <a href="npc">Maligaro</a> grab the <a href="important">boot reward</a> from <a href="npc">Helena</a> in town that matches your builds armor type:
  - For Righteous Fire look for <a href="b">BBB</a><a href="r">R</a> links.
  - For Fire Trap look for <a href="g">GGG</a><a href="r">R</a> links.
- If the reward has exactly those links, take it. It's ready to equip with your corresponding gems once you swap builds.
  - Check wiki if needed: <a href="[filename](https://www.poewiki.net/wiki/Essence_of_Artist)">Essence of Artist Quest</a>
- Start looking for sceptres and shields and links for your curse and movement setups:
  - For Curse look for <a href="b">BBB</a><a href="r">R</a> links. (AR/ES Base for easy color)
  - For Movement look for <a href="g">GG</a><a href="r">R</a><a href="b">B</a> links. (AR/ES Base for easy color)

</ul>
</div>

## <span id="a8"></span>ACT 8 PROGRESSION
### End of Act 8: 2h30, Level 52

<img src="327_sunder/images/act8.png">

<div class="note-block">
<ul class="custom-list bullet-list">

- Tree above is what you should have going from Act 1 to end of Act 8 (Blood Aqueducts)
- This is the "pre swap" tree at around level 52 or 55

</ul>
</div>

---

## <span id="respec"></span>LEVEL 52 RESPEC (BA)
### Time at BA: 2h30, Level 52

<div class="note-block">
<ul class="custom-list bullet-list">

- Use Campaign respec points first (~ 14 pts)
- Can get 16 pts if you do Fetid Pool
- Spend gold to finish the respec according to tree below
- Remove points in <a href="r">Red</a> and take <a href="g">Green</a> on tree below.
  
</ul>
</div>
<img src="327_sunder/images/final.png">

<div class="note-block">
<ul class="custom-list bullet-list">

- 26 point respec require d= 16 from campaign + 10 from gold
  - 10 pts @ 313 gold each (lvl 52) = 3130 gold
  - In a test run (lvl 52 @ BA) I ended with 7282 gold after doing 3 armor gambles at level 24 in act 3 to get 1 4-link.
  - After 26 point respect I ended with 4152 gold which I used to gamble other armor pieces or sceptre/shield for RF swap.
- Gamble body (705 gold), gloves/helm (676 gold) for 4-link RF/FT colors if you did not get from quest rewards in A6 or A7.
- As a reminder these are the colors you're looking for:
  - Righteous Fire - <a href="b">BBB</a><a href="r">R</a>
  - Fire Trap - <a href="g">GGG</a><a href="r">R</a>
  - Curse - <a href="b">BBB</a><a href="r">R</a>
  - Movement - <a href="g">GG</a><a href="r">R</a><a href="b">B</a>
  
</ul>
</div>

<div class="imp-block" id="a9links">
<ul class="checklist">
If you're continuing into Heist or finishing campaign it's important to buy these 9 gems to level off hand or in any open sockets or off-hand. Ensure you buy 2 Lifetaps from Act 1 so they are pre-leveled and another 2 from Act 6 to keep at level 1. If you're interested where they are used, check the Heist links below.

- [ ] Act 1: [2x Lifetap (max)](gemred)2x <img src="images/alt.png" class="icon-inline">, [Enduring Cry](gemred) <img src="images/trans.png" class="icon-inline">, [Vitality](gemred) <img src="images/trans.png" class="icon-inline">
- [ ] Act 3: [Punishment](gemred)<img src="images/alt.png" class="icon-inline">
- [ ] Act 6: [2x Lifetap (1)](gemred)2x <img src="images/trans.png" class="icon-inline">, [Arrogance](gemred)<img src="images/chance.png" class="icon-inline">, [Skitterbots](gemblue)<img src="images/chance.png" class="icon-inline">

Total Cost: 6 Transmutes <img src="images/trans.png" class="icon-inline">, 2 Alterations <img src="images/alt.png" class="icon-inline">, 1 Chance <img src="images/chance.png" class="icon-inline">

</ul>
</div>

---

## <span id="rfheist"></span>RF HEIST LINKS
Try to have these links as soon as you get to BA and start doing heists. You can swap around the auras to fit your needs. For example, if you think you're tanky enough or if you have enough damage, feel free to replace determination or malevolence with [Haste](gemgreen) so you can move faster.
<div class="link-block" id="rfheist">
<ul class="checklist">

- [ ] Link 1: [RF](gemblue)+[Elemental Focus](gemblue)+[Lifetap (max)](gemred)+[Burning Damage](gemred) or [Efficacy](gemblue)
- [ ] Link 2: [Fire Trap](gemgreen)+[Swift Affliction](gemgreen)+[Lifetap (max)](gemred)+[Trap & Mine Damage](gemgreen)
- [ ] Link 3: [Shield Charge](gemred)+[Lifetap (1)](gemred)+[Faster Attacks](gemgreen)+[Momentum](gemgreen)
- [ ] Link 4: [Vitality](gemred)+[Arrogance](gemred)
- [ ] Link 5: [Punishment](gemred)+[Lifetap (1)](gemred)+[Enduring Cry](gemred)+[Frostblink (1)](gemblue)
- [ ] Aura: [Determination](gemred), [Malevolence](gemblue), [Skitterbots](gemblue)
- [ ] Util: [Blood Rage](gemgreen)

</ul>
</div>

---

## <span id="rfmap"></span>RF MAPPING LINKS
- The RF gems are in order of importance, some gems like Swift Affliction will not work without the use of Lifetap as it adds a duration tag.
- It will be important to buy aura gems directly from A3 so they start at Level 11. Getting Purity of Fire and Determination first is really big for survivability. 
- If you have enough damage or defences, you can use Haste to move faster.
<div class="link-block" id="rfmap">
<ul class="checklist">

- [ ] Link 1: [RF](gemblue)+[Burn Damage](gemred)+[Ele Focus](gemblue)+[Efficacy](gemblue)+[Lifetap (max)](gemred)+[Swift Aff](gemgreen)
- [ ] Link 2: [Fire Trap](gemgreen)+[Swift Affliction](gemgreen)+[Lifetap (max)](gemred)+[Trap & Mine Damage](gemgreen)
- [ ] Link 3: [Shield Charge](gemred)+[Lifetap (1)](gemred)+[Faster Attacks](gemgreen)+[Frostblink (1)](gemblue)
- [ ] Link 4: [Vitality](gemred)+[Arrogance](gemred)
- [ ] Link 5: [Punishment](gemred)+[Lifetap (1)](gemred)+[Enduring Cry](gemred)
- [ ] Aura: [Determination](gemred), [Purity of Fire, ](gemred)[Malevolence](gemblue) or [Skitterbots](gemblue)
- [ ] Util: [Blood Rage](gemgreen)

</ul>
</div>
<!-- END OF MAIN CONTENT -->
<!-- DONT TOUCH BELOW -->

---
<footer class="main-footer">
<div class="footer-content">
    <div class="footer-left">
        <p>&copy; 2026 Muzaroni Archive</p>
        <p class="footer-subtext">is not affiliated with or endorsed by Grinding Gear Games.</p>
    </div>
    <div class="footer-right">
        <a href="#" id="back-to-top">Back to Top ↑</a>
        <a href="https://pathofexile.com" target="_blank">Official PoE</a>
    </div>
</div>
</footer>
</article>
</div>
<!-- DONT TOUCH ABOVE -->
<aside class="toc-container">
    <nav class="toc">
        <h4>On This Page</h4>
        <ul>
            <li><a href="#pob">POB LINKS</a></li>
            <li><a href="#regex">REGEX</a></li>
            <li><a href="#a1">ACT 1</a></li>
            <li class="sub-item"><a href="#a1links">A1 Links</a></li>
            <li><a href="#a2">ACT 2</a></li>
            <li class="sub-item"><a href="#a2links">A2 Links</a></li>
            <li><a href="#a3">ACT 3</a></li>
            <li class="sub-item"><a href="#a3links">A3 Links</a></li>
            <li><a href="#a4">ACT 4</a></li>
            <li class="sub-item"><a href="#a4links">A4 Links</a></li>
            <li><a href="#a5">ACT 5</a></li>
            <li class="sub-item"><a href="#a5links">A5 Links</a></li>
            <li><a href="#a6">ACT 6</a></li>
            <li class="sub-item"><a href="#a6links">Gems to buy in A6</a></li>
            <li><a href="#a7">ACT 7</a></li>
            <li><a href="#a8">ACT 8</a></li>
            <li><a href="#respec">Level 52 Respec</a></li>
            <li class="sub-item"><a href="#a9links">Gems to buy in A9</a></li>
            <li><a href="#rfheist">HEIST</a></li>
            <li><a href="#rfmap">MAPPING</a></li>
        </ul>
    </nav>
</aside>
<!-- DONT TOUCH BELOW -->
    </div>
    </main>
</div>

<link rel="stylesheet" href="style.css">
<script src="script.js"></script>
